...
test('exposes all editor views and conversion options as sidebar radios', () => {
  ...
  assert.doesNotMatch(detailsSection('Tidy'), /Normalize inequality commands|Normalize not-equal command/)
  assert.equal((detailsSection('Tidy').match(/name="(?:greaterEqualCommand|lessEqualCommand|notEqualCommand)"/g) ?? []).length, 3)
  ...
  assert.doesNotMatch(detailsSection('Tidy'), /<label for="(?:ge|le|ne)-command">/)
